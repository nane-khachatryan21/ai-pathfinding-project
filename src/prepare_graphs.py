"""
Script to prepare and save graph data files.
...
3. Yerevan - complete Yerevan road network
"""
...
def prepare_armenia_cities():
    ...
    try:
        ...
        G = ox.graph.graph_from_polygon(poly, network_type="drive", simplify=True, custom_filter=CUSTOM_FILTER)
        ...
        for u in labels:
            neigh = sorted(((v, gc_km(coords[u], coords[v])) for v in labels if v != u), key=lambda x: x[1])[:k]
...
def prepare_armenia_cities_villages():
    """
    Prepare Armenian cities and villages graph
    """
    ...
    try:
        ...
        # Get settlements (cities/towns and villages)
        print("Getting cities, towns, and villages...")
        sett = ox.features.features_from_polygon(poly, tags={"place": ["city", "town", "village"]})
        sett = sett[~sett.geometry.is_empty].copy()
        ...
        print(f"Cities and Villages graph: {H.number_of_nodes()} nodes, {H.number_of_edges()} edges")
        ...
        print(f"Error preparing Armenia cities and villages graph: {e}")
        return None


def prepare_yerevan():
    """
    Prepare Yerevan graph
    """
    ...
    try:
        ...
        # Clean node attributes to keep only essential data
